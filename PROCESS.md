...
공부하다 막힌 것을 그 자리에서 물어보는 기능. 퀴즈 화면에서는 옆 사이드바로, 학습
화면에서는 떠 있는 버튼으로 붙는다. 답변의 근거는 그 세션이 가진 것뿐이다 — 올린
강의자료, 분석으로 만든 학습 주제, 시험 범위.

### 설계 판단

**"답하는 것"보다 "무엇에 근거해 답했는지"가 어렵다.** 자료를 올린 사람은 답변이 자기
강의자료에서 나왔다고 믿는다. 그래서 두 값을 나눠 내려준다.
...
자료를 올린 세션이라도 질문이 자료 밖이면 두 번째는 false 다. 그때 화면은 "자료에서
근거를 찾지 못해 일반 지식으로 답했다"를 띄운다. 자료가 없는 세션에서는 모델이 규칙을
어기고 true 로 답해도 서버가 그대로 내보내지 않는다.

**대화 기록은 서버가 갖는다.** 이유가 두 개다. 8자리 코드로 다른 기기에서 이어져야
하고, 클라이언트가 지난 대화를 보내오게 두면 없던 발화를 지어내 프롬프트에 넣을 수 있다.

**경로를 세션 아래에 뒀다.** 기획 초안은 `POST /api/chat/study` 였다. 이 서비스에는
회원이 없고 8자리 코드가 유일한 접근 키라, 세션 밖의 경로에서는 이 대화가 누구 것인지
확인할 방법이 없다. 퀴즈 재생성 API 를 세션 아래에 둔 것과 같은 이유다.

### 진행 순서

1. `ChatMessage` / `ChatRole` 엔티티, 리포지터리
2. `SessionPurgeRepository` 삭제 순서에 추가 — 빠뜨리면 만료 세션 정리가 통째로 실패한다
3. `StudyContextProvider` 인터페이스 + `SimpleStudyContextProvider`
4. `AiChatClient` 와 네 구현체(Claude / Gemini / Mock / Unavailable), `AiChatClientConfig`
5. `ChatPrompts` — 주입 방어 태그 네 개, 근거 규칙 두 갈래
6. `ChatService`, DTO, `ChatController`, 에러 코드 2종
7. 테스트 31건 (실제 AI 를 부르지 않는다)
8. `docs/migrations/002-study-chat.sql`, `docs/schema.sql` 갱신
9. 프론트 — `useStudyChat`, `StudyChat`(사이드바 / 떠 있는 버튼), 퀴즈·학습 화면에 연결
10. 검증 (`scripts/chat-verify.sh`) → `docs/study-chat-test.md`

### 판단이 갈렸던 곳

| 갈림길 | 고른 쪽 | 버린 쪽과 그 이유 |
| --- | --- | --- |
| 지난 대화 전달 | `<conversation>` 태그 안 | SDK 멀티턴 자리 — 저장된 대화가 "실제 발화"로 취급되어 주입 여지가 커진다 |
| 컨텍스트 추출 | 챗봇 전용 구현 | `QuizContextExtractor` 재사용 — 퀴즈는 Topic 기준으로 넓게, 챗봇은 질문 기준으로 좁게 잡는다. 한 클래스에 두 기준을 넣으면 한쪽을 고칠 때마다 다른 쪽이 흔들린다 |
| 관련 구간을 못 찾았을 때 | 빈 구간을 보낸다 | 앞부분이라도 담기 — 퀴즈는 "그 Topic 이 그 문서에서 나왔다"는 근거가 있지만 챗봇에는 없다. 관계없는 문단을 근거랍시고 붙이면 그럴듯한 오답이 나온다 |
| 저장 시점 | 성공했을 때 한 쌍으로 | 질문 먼저 저장 — 실패하면 답 없는 질문만 남고 다음 프롬프트에 섞인다 |
| 학습 화면의 배치 | 떠 있는 버튼 | 사이드바 — 학습 화면은 좌우 칸을 이미 쓰고 있어 네 칸이 된다 |

### 겪은 문제

| 문제 | 원인 | 조치 |
| --- | --- | --- |
| `GET /chat` 이 대화가 없으면 `grounded=false` 를 내보냈다 | 첫 메시지의 세션에서 값을 꺼내려 했다. 대화가 없으면 꺼낼 곳이 없고, 지연 로딩이라 트랜잭션 밖에서는 초기화도 실패한다 | 서비스가 `ChatHistory(grounded, messages)` 로 함께 돌려준다 |
| 새 lint 규칙(`react-hooks/set-state-in-effect`) 위반 | effect 본문에서 곧바로 `setState` | 응답 콜백에서만 상태를 바꾸고, "세션 코드 없음"은 상태 대신 계산으로 |

### 확인

- `./gradlew test` — 903건 통과 (챗봇 31건 포함)
