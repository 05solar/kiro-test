...
- [학습 맥락 반영](#학습-맥락-반영)
- [자료가 없는 경우 — 일반 지식 기반 생성](#자료가-없는-경우--일반-지식-기반-생성)
...
| `PARSED` 자료가 1개 이상 있거나, **과목명과 시험 범위가 모두 있다** | 400 `NO_PARSED_DOCUMENT` |
| 이미 분석 중이 아니다 | 409 `ANALYSIS_ALREADY_RUNNING` |

`PARSE_FAILED` 자료가 섞여 있어도 분석은 가능하다. `PARSED` 자료만 대상으로 삼고
제외한 파일은 서버 로그로만 남긴다.

자료가 하나도 없어도 분석할 수 있다. 그 경우 과목명과 시험 범위만 보고 일반적인 교과
지식에서 주제를 만든다 — [자료가 없는 경우](#자료가-없는-경우--일반-지식-기반-생성) 참고.
둘 중 하나라도 비어 있으면 만들 근거가 아무것도 없으므로 `NO_PARSED_DOCUMENT` 로 실패한다.
...
| 400 | `NO_PARSED_DOCUMENT` | 추출된 자료도 없고, 대신 쓸 시험 범위(또는 과목명)도 없음 |
...
## 자료가 없는 경우 — 일반 지식 기반 생성

강의자료 없이도 벼락치기는 가능해야 한다. 자료가 손에 없거나, 과목명과 범위만 아는
상황이 실제로 흔하다. 그래서 추출된 자료가 하나도 없으면 분석을 거절하지 않고
**과목명과 시험 범위만 보고** 일반적인 교과 지식에서 주제를 만든다.

```
추출된 자료 있음  →  sourceType = USER_MATERIAL      (자료에서 뽑는다)
추출된 자료 없음  →  sourceType = GENERAL_KNOWLEDGE  (과목명 + 시험 범위로 만든다)
```

무엇에 근거했는지는 세션에 남는다. `GET /api/sessions/{code}` 의 `sourceType` 과
`grounded` 로 확인한다 — 자세한 정의는 [세션 API](session-api.md#sourcetype--학습-내용의-근거).

### 이 경로에서 달라지는 것

| | 자료 기반 | 일반 지식 기반 |
| --- | --- | --- |
| 자료 조각 분석 | 조각 수만큼 AI 호출 | **하지 않는다** (나눌 자료가 없다) |
| 근거 | 추출한 텍스트 | 과목명 + 시험 범위 + 학습 맥락 |
| `sourceDocuments` | 실제 문서 ID | 항상 비어 있다 |
| 퀴즈 출제 근거 | 자료 추출 구간 | Topic 의 제목·요약·핵심 개념 |

**AI 가 출처 문서를 지어내도 버린다.** 일반 지식으로 만든 Topic 에는 근거로 삼을 문서가
없으므로, 응답에 문서 ID 가 들어 있으면 검증 단계에서 전부 제거한다.

### 사용자에게 감추지 않는다

이 경로로 만든 내용은 실제 수업 범위와 다를 수 있다. 화면이 그 사실을 감추면 사용자는
자기 강의자료에서 뽑은 내용이라고 믿고 그대로 외운다. 그래서 커리큘럼·퀴즈 화면에
근거를 항상 표시한다.
...
아래는 강의자료가 있는 경우(`USER_MATERIAL`)의 정책이다. 자료가 없는 경우는 애초에
대조할 자료가 없으므로 위 절이 대신한다.
