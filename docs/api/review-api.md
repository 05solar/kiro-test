...
세 자리에서 쓸 수 있고 셋 다 같은 방식이다.

| 화면 | 대상 | 문제를 얻는 곳 |
| --- | --- | --- |
| 퀴즈 내역 (STEP 하나) | 그 STEP에서 틀린 문제 | `GET /topics/{topicId}/quiz-review` |
| 전체 정리 — 푼 문제 전체 | 모든 STEP에서 푼 문제 | `GET /review` |
| 전체 정리 — 틀린 문제만 | 모든 STEP에서 틀린 문제 | `GET /review` |

전체 정리 쪽은 STEP을 가로질러 모은다 — STEP 순서, 그 안에서는 문제 순서다.
문제마다 어느 STEP 것인지 꼬리표가 붙는다. 여러 STEP을 섞어 풀면 문제만 보고는
무엇에 대한 질문인지 잡기 어렵기 때문이다.

새 요청을 보내지 않는다. 이미 받아 둔 응답에서 골라 쓴다.
