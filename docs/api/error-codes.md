...
| `NO_PARSED_DOCUMENT` | 400 | 만들 근거가 없습니다. 강의자료를 올리거나, 시험 범위를 입력해 주세요. | 추출된 자료도 없고, 대신 쓸 시험 범위(또는 과목명)도 없이 AI 분석 요청 |
...
| `CHAT_NOT_READY` | 409 | 학습 내용을 먼저 만들어 주세요. 아직 답할 근거가 없습니다. | 분석 전 세션에 챗봇 질문 |
...
| `CHAT_FAILED` | 502 | 답변을 만들지 못했습니다. 다시 시도해 주세요. | AI 호출 실패, 빈 응답, 응답 형식 오류 |
